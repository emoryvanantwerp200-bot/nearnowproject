...
export default async (req: Request) => {
  ...
  const store = getStore("daily-briefs");
  const latest = (await store.get("latest", { type: "json" }).catch(() => null)) as
    | { summaries?: Record<string, string>; generatedAt?: string }
    | null;
  const summary = latest?.summaries?.[area] || fallbackSummaries[area] || fallbackSummaries.mobile;

  return Response.json({
    summary,
    generatedAt: latest?.generatedAt || null,
    source: latest?.summaries?.[area] ? "scheduled-ai-brief" : "fallback",
  });
};
